"""Graph Intelligence Router — Agent behavior graph (auth-protected, user-scoped)."""

from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import select, func
from database import get_db
from models import Agent, AuditLog, Violation, RiskLevel, User
...
async def get_graph_nodes(
    ...
    result = await db.execute(select(Agent).where(Agent.owner_id == user.user_id))
    ...
    for a in agents:
        nodes.append({
            "id": a.agent_id, "label": a.agent_name, "type": "agent",
    ...
    for t in tools_set:
        nodes.append({"id": f"tool:{t}", "label": t, "type": "tool", "size": 8})
    for d in data_sources_set:
        nodes.append({"id": f"data:{d}", "label": d, "type": "data_source", "size": 8})
...
async def get_graph_edges(
    ...
    for a in agents:
        for t in (a.allowed_tools or []):
            edges.append({"source": a.agent_id, "target": f"tool:{t}", "type": "uses_tool", "weight": 1})
        for d in (a.allowed_data_sources or []):
            edges.append({"source": a.agent_id, "target": f"data:{d}", "type": "accesses_data", "weight": 1})

    if agent_ids:
        v_result = await db.execute(
            select(Violation).where(Violation.agent_id.in_(agent_ids)).limit(200)
        )
        ...
        for v in violations:
            ...
            if key not in violation_edges:
                violation_edges[key] = {
                    "source": v.agent_id, "target": f"violation:{v.violation_id[:8]}",
                    "type": "violation", "weight": 0, "violations": [],
                }
...
async def get_blast_radius(
    ...
    # Only show connected agents that belong to this user
    result = await db.execute(
        select(Agent).where(Agent.owner_id == user.user_id).where(Agent.agent_id != agent_id)
    )
    ...
    for other in all_agents:
        ...
        if common_tools or common_data:
            connected_agents.append({
                "agent_id": other.agent_id, "agent_name": other.agent_name,
                "department": other.department,
                "shared_tools": list(common_tools), "shared_data_sources": list(common_data),
    ...
    return {
        ...
        "blast_radius": {
            "risk_rating": risk_rating, "direct_tools": list(direct_tools),
            "direct_data_sources": list(direct_data), "connected_agents": connected_agents,
            "total_connected_agents": len(connected_agents),
            "shared_tools": shared_tools, "shared_data_sources": shared_data,
